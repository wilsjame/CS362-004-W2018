...
int Remodel(struct gameState *state,int currentPlayer,int choice1,int choice2, int handPos){
	int i,j;
	j = state->hand[currentPlayer][choice1];  //store card we will trash
	...
	int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
		 return Adventurer(drawntreasure,cardDrawn,z,state,currentPlayer,temphand);
    ...
    case mine:
		return Mine(state,currentPlayer,choice1,choice2,handPos);
    
	 case remodel:
		return Remodel(state,currentPlayer,choice1,choice2,handPos);
